...
void manejador(int senal){
	...
	for(int i=0; i<=list_size(tareas);i++){
		...
		if(pid_wait==item->pgid){
			...
			if(status_res==SUSPENDIDO){
				printf("/n Comando %s ejecutado en segundo plano con PID %d ha suspendido su ejecucion /n", item->command,item->pgid);
			item->ground=DETENIDO;
			}else if(status_res==FINALIZADO){
					printf("/n Comando %s ejecutado en segundo plano con PID %d ha concluido /n", item->command,item->pgid);
...
int main(void)
{
      ...
      int info;	
	  int primerplano=0;      // Informaci—n procesada por analyze_status()
      job*item;
    ...
    if(pid_fork > 0){
    	if (background == 0){
			...
			unblock_SIGCHLD();
				...
				if(info != 255){
        		printf(" Comando %s ejecutado en primer plano con pid %d. Estado %s. Info: %d ", args[0], pid_fork, status_strings[status_res],info);
			}
			}
        	primerplano=0;
		}else{
